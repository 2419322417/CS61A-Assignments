...
def count_dollars_upward(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def count_quantity(total, smallest_bill):
        if total == 0:
            return 1
        if total < 0:
            return 0
        if smallest_bill == None:
            return 0
        with_dollar_bill = count_quantity(total - smallest_bill, smallest_bill)
        without_dollar_bill = count_quantity(total, next_larger_dollar(smallest_bill))
        return with_dollar_bill + without_dollar_bill
    return count_quantity(total, 1)
